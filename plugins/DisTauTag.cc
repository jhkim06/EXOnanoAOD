...
private:
    ...
    void saveInputs(const tensorflow::Tensor& tensor, const std::string& block_name, 
                   const edm::EventID& eventId, size_t jetIndex) const;
...
void DisTauTag::saveInputs(const tensorflow::Tensor& tensor, const std::string& block_name,
    ...
    std::string json_file_name = "distag_"
            + std::to_string(eventId.run()) + "_"
            + std::to_string(eventId.luminosityBlock()) + "_"
            + std::to_string(eventId.event()) + "_"
    ...
    for(int tau_idx=0; tau_idx<tau_n; tau_idx++) {
        ...
        for(int pf_idx=0; pf_idx<pf_n; pf_idx++) {
            ...
            for(int ftr_idx=0; ftr_idx<ftr_n; ftr_idx++) {
                json_file << std::setprecision(7) << std::fixed 
                         << tensor.tensor<float, 3>()(tau_idx, pf_idx, ftr_idx);
                if(ftr_idx<ftr_n-1) json_file << ", ";
            }
            json_file << "]";
            if(pf_idx<pf_n-1) json_file << ", ";
        }
        json_file << "]";
        if(tau_idx<tau_n-1) json_file << ", ";
    }
    json_file << "]}";
}
        ...
        for(size_t batch_jet_idx = 0; batch_jet_idx < current_batch_size; ++batch_jet_idx) {
            ...
            while(tensor_idx < static_cast<size_t>(Setup::nSeq_PfCand) && daughter_idx < nDaughters) {
                ...
                {   // General features
                    ...
                    getVecRef(input_1, PfCand_Features::pfCand_deta, 
                             static_cast<Float_t>(daughter->polarP4().eta()) - jet_eta);
                    getVecRef(input_1, PfCand_Features::pfCand_dphi, 
                             getDeltaPhi<Float_t>(static_cast<Float_t>(daughter->polarP4().phi()), jet_phi));
                }

                {   // Categorical features
                    typedef PfCandCategorical_Features Br;
                    getVecRef(input_2, Br::pfCand_particleType, 
                             static_cast<Int_t>(TranslatePdgIdToPFParticleType(daughter->pdgId())));
                    getVecRef(input_2, Br::pfCand_pvAssociationQuality, 
                             static_cast<Int_t>(daughter->pvAssociationQuality()));
